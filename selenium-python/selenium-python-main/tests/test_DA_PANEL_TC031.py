...
class test_DA_PANEL_TC031(TestBase):
    ...
    def test_DA_PANEL_TC031(self, user_name, password):
        ...
        SoftAssert.soft_assert(lambda: self.assertEqual(self.new_panel_dialog.get_panel_setting_form_title(),"Chart Settings" ,"Panel setting form is not displayed 'Chart Setting'"))
        SoftAssert.soft_assert(lambda: self.assertTrue(self.new_panel_dialog.is_panel_setting_form_displayed_under_display_name_field(), "Chart panel setting form is not displayed under Display Name field"))

        self.new_panel_dialog.select_panel_type(PanelType.INDICATORS)
        SoftAssert.soft_assert(lambda: self.assertEqual(self.new_panel_dialog.get_panel_setting_form_title(),"Indicator Settings" ,"Panel setting form is not displayed 'Indicator Setting'"))

        self.new_panel_dialog.select_panel_type(PanelType.HEAT_MAPS)
        SoftAssert.soft_assert(lambda: self.assertEqual(self.new_panel_dialog.get_panel_setting_form_title(),"Heat Map Settings" ,"Panel setting form is not displayed 'Heat Map Setting'"))
